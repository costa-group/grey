...
def delete_unsued_instructions_from_deps(sto_deps, mem_deps, trans_deps, gas_deps, unused_ids):
    ...
    new_trans_deps = [x for x in trans_deps if x[0] not in unused_ids and x[1] not in unused_ids]
    new_gas_deps = [x for x in gas_deps if x[0] not in unused_ids and x[1] not in unused_ids]
    return new_sto_deps, new_mem_deps, new_trans_deps, new_gas_deps
