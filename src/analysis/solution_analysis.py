...
def function_frequency(cfg: CFG, cfg_name: str) -> List[Dict]:
    """
    ...
    for object_id, cfg_object in cfg.objectCFG.items():
        ...
        current_info.append({"cfg_name": cfg_name, "object": object_id, "num_functions": len(function_names),
                             "num_calls": sum(function2info.values())})

        sub_object = cfg_object.get_subobject()
        if sub_object is not None:
            current_info.extend(function_frequency(sub_object, cfg_name))
