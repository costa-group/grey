...
def compute_pos_ops(deps: List[Tuple[id_T, id_T]], pos: Dict[id_T, int]) -> Tuple[List[Tuple[int, List[id_T]]], List[Tuple[int, List[id_T]]]]:
    """
...
def merge(morder, sorder, final_no_mstore, final_no_sstore, opid_instr_map: opid_instr_map_T, var_instr_map: var_instr_map_T):
    torder = []
    if len(morder) == 0:
        return sorder
    if len(sorder) == 0:
...
def needed_list(v: Union[var_T, instr_T], lops: List[var_T], needed_set: Set[var_T], opid_instr_map: opid_instr_map_T, var_instr_map: var_instr_map_T) -> int:
    """
...
def needed_nostores(msops: List[id_T], final_stack: List[var_T], opid_instr_map: opid_instr_map_T, var_instr_map: var_instr_map_T):
    """
...
def add_needed_nostores_in_stack(nostores, torder, opid_instr_map: opid_instr_map_T, var_instr_map: var_instr_map_T, mem_order, sto_order):
    if len(nostores) == 0:
        return torder
    else:
        i = 0
...
class SMSgreedy:
    ...
    def duplicate(self, o: var_T):
        ...
        compute
        """
        return o in self._forced_in_stack or o in self._initial_stack or not self.small_zeroary(o) or not self.easy_compute(o)
    ...
    def precompute(self, final_stack: List[var_T], current_stack: List[var_T]) -> Tuple[List[str], List[id_T], List[var_T], List[var_T]]:
        """
        ...
        while len(current_stack) > 0:
            ...
            if current_stack[0] not in self._needed_in_stack_map or self._needed_in_stack_map[current_stack[0]] == 0:
                ...
                if len(current_stack) > len(final_stack) and current_stack[0] in final_stack:
                    pos = final_stack.index(current_stack[0]) - len(final_stack)  # position from the end in current_stack
                elif len(current_stack) <= len(final_stack) and current_stack[0] in final_stack[-len(current_stack):]:
                    pos = final_stack[-len(current_stack):].index(current_stack[0]) - len(current_stack)  # position from the end in current_stack
                else:
                    break
                ...
                if pos_in_stack > 0:
                    ...
                    current_stack = [current_stack[pos_in_stack]] + current_stack[1:pos_in_stack] + [current_stack[0]] + current_stack[pos_in_stack + 1:]
    ...
    def _compute_top_can_used(self, instr: instr_T, top_can_be_used: Dict):
        ...
        """
        identifier = instr["outpt_sk"][0] if len(instr["outpt_sk"]) > 0 else instr["id"]
        top_elements = top_can_be_used.get(identifier, None)

        # Top elements have already been computed previously
        ...
        for stack_var in reversed(instr["inpt_sk"]):
            ...
            if comm or first_element:
                instr_bef = self._var_instr_map.get(stack_var, None)
        ...
        top_can_be_used[identifier] = current_uses
        return current_uses
    ...
    def must_reverse(self, o: Union[var_T, id_T], inpts: List[var_T], stack: List[var_T], needed_stack: Dict):
        ...
        if stack[0] in self._top_can_be_used[inpts[1]] and stack[0] in self._top_can_be_used[inpts[0]]:
            return False

        return True

    def compute_one_with_stack(self, o: Union[var_T, id_T], stack: List[var_T], needed_stack: Dict, solved: List[int], max_to_swap: int):
...
#                                print("here1")
            ...
            if o in self._var_instr_map and self._var_instr_map[o]['commutative']:
                inpts.reverse()
                (opcodes, opcodeids, stack) = self.compute_with_stack_permut(inpts, stack, needed_stack, solved, max_to_swap)
            else:
    ...
    def compute_with_stack_permut(self, inpts, stack, needed_stack, solved, max_to_swap):
        ...
        # Permutate the inputs if there is an element that can be reused
        if len(stack) > 0 and stack[0] in self._top_can_be_used.get(inpts[1], set()) and \
                stack[0] not in self._top_can_be_used.get(inpts[0], set()):
            inpts.reverse()
            ...
            if self._dup_stack_ini == i and i < len(stack) and stack[i] in inpts[i:]:
                ...
                if i_in_final not in solved:
                    if op in needed_stack and needed_stack[op] == 1:
                            ...
                            i += 1
                            continue
            nopcodes, nopcodeids, stack = self.compute_one_with_stack(inpts[i], stack, needed_stack, solved, max_to_swap)
            opcodes += nopcodes
    ...
    def choose_element(self, solved, cstack, needed_stack) -> Tuple[int, int]:
        ...
        while pos >= 0:
            ...
            if pos not in solved and cstack.count(self._final_stack[pos]) < self._final_stack.count(self._final_stack[pos]):
    ...
    def clean_stack(self, o: Union[var_T, id_T], stack: List[var_T], needed_stack: Dict, solved: List[int]) -> Tuple[List[id_T], List[var_T], Dict]:
        """
    ...
    def compute_pre_list(self, lord: List[var_T], cstack: List[var_T],
        ...
        for op in lord:
            ...
            comm = self._var_instr_map[op]["commutative"]

            if comm:
                popcodes, popcodeids, cstack = self.compute_with_stack_permut(inpts, cstack, cneeded_in_stack_map,
                                                                              solved, max_to_swap)
    ...
    def compute(self, dependency_instrs: List[Union[id_T, var_T]], final_no_store: List[var_T], opcodes_ini: List[str],
        ...
        while case != 2:
            ...
            # Otherwise, we choose the case and pos from choose element
                ...
                if VERBOSE > 1:
                    print("Choose element chose case", case, "with pos", pos)
                # print('case:', case, pos)
                        ...
                        if dependency_instrs[i] == o:
                            ...
                            for i in range(p, len(dependency_instrs)):
                                if "STORE" in dependency_instrs[i]:
                                    before_store = True
                                    break
                            dependency_instrs = dependency_instrs[:p] + dependency_instrs[p + 1:]  # remove the operation
                        else:
                            p = i + 1
