...
class SMSgreedy:
    ...
    def must_reverse(self, o, inpts, stack, needed_stack):
        ...
        if not self._var_instr_map[o]['commutative']:
            return True
        if self.needs_in_stack_too_far(inpts[0], stack, needed_stack) >= reach:
            return False
    ...
    def compute_one_with_stack(self, o, stack, needed_stack, solved, max_to_swap):
        ...
        if o in self._var_instr_map:
            if self.small_zeroary(o) and o not in self._dependences_done:
                ...
                if verbose: print(opcodeid, [o] + stack, len([o] + stack))
                return ([opcode], [opcodeid], [o] + stack)
        if not (o not in needed_stack or o in stack[:reach]):
            if self._dup_stack_ini == 0:
                self.clean_stack(o, stack, needed_stack, solved)
        if o in needed_stack and o not in stack[:reach]:
            ...
            if pos not in solved and cstack.count(self._final_stack[pos]) < self._final_stack.count(
                ...
                while pos1 >= 0 and len(cstack) + pos1 - len(self._final_stack) >= 0:
                    if pos1 not in solved and cstack.count(self._final_stack[pos1]) < self._final_stack.count(
                        ...
                        if ns <= ns1 and reach-1 <= ns1 and ns1 <= reach:
                            pos = pos1
                            break
                    pos1 -= 1
                # print("case 1",pos)
    ...
    def stack_too_long(self, stack, mem, needed_stack):
        ...
        for o in mem:
            if not is_write(o):
                num_no_store += 1
        if num_no_store >= reach-1:
            return False
        while (i + num_no_store >= reach-1):
    ...
    def clean_stack(self, o, stack, needed_stack, solved):
        ops = []
        i = 0
        if self.needs_in_stack_too_far(o, stack, needed_stack) <= reach - 2:
            return ([], stack, needed_stack)
        while i < len(stack) and len(stack) >= reach - 6 and i <= reach:
            while i < len(stack) and i <= reach and (len(self._final_stack) + i - len(stack)) not in solved:
                if stack[i] in needed_stack and needed_stack[stack[i]] == 0 and \
                   stack.count(stack[i]) > self._final_stack.count(stack[i]):
                    break
                i += 1
            if i > 0 and i < len(stack) and i <= reach and (len(self._final_stack) + i - len(stack)) not in solved:
                opr = stack[i]
                ops += ['SWAP' + str(i)]
...
#                cstack = [self._opid_instr_map[o]["outpt_sk"][0]] + cstack
        ...
        while case != 2:
            ...
            if case == 0:
                ...
                if (cstack[0] == cstack[pos]):
                    ...
                    assert (pos_in_stack > 0 and pos_in_stack <= reach) #when case 0 should not happen
                ...
                # assert (pos_in_stack >= 0)
                # assert (pos_in_stack <= reach)
                # added only to avoid being removed when cannot be place in its position because is beyond "reach" ****
                if (pos_in_stack < 0 or pos_in_stack > reach) and (o not in cneeded_in_stack_map or cneeded_in_stack_map[o] == 0):
                    cneeded_in_stack_map[cstack[0]] = 1
                # end of added code
                if pos_in_stack > 0  and  pos_in_stack <= reach: #othewise leave it on top
                    topcodes += ['SWAP' + str(pos_in_stack)]
                    topcodeids += ['SWAP' + str(pos_in_stack)]
                    lens = len(cstack)
                    cstack = [cstack[pos_in_stack]] + cstack[1:pos_in_stack] + [cstack[0]] + cstack[pos_in_stack + 1:]
    ...
    def first_unsolved_position(self,n,cstack,solved):
        i = n
        k = 1
        while k <= reach and k < len(cstack) and i not in sloved:
            k += 1
            i += 1
        return k
    ...
    def compute_permut_and_clean(self, cstack, solved):
        ...
        while cstack != self._final_stack:
            ...
            if allelems.count(cstack[0]) > self._final_stack.count(cstack[0]):
                ...
                cstack.pop(0)
                if verbose: print('POP',cstack,len(cstack))
            elif len(self._final_stack) >= len(cstack) and len(self._final_stack)-len(cstack) not in solved and cstack[0] == self._final_stack[len(self._final_stack)-len(cstack)] and not self.unsolved(cstack[0],len(self._final_stack)-len(cstack)+1, reach, solved):
                #the first position in cstack is not marked as solved but is solved
                # print("Top is solved",self.unsolved(cstack[0],len(self._final_stack)-len(cstack)+1, reach, solved))
                solved.append(len(self._final_stack)-len(cstack))
            elif len(self._final_stack)-len(cstack) not in solved or self.unsolved(cstack[0],len(self._final_stack)-len(cstack)+1, reach, solved):
                #the first position in cstack is not solved
                i = min(reach,len(cstack)-1) #search last reachable not solved that is different from top
                ...
                if i >= 1 and len(self._final_stack)-len(cstack)+i >= 0: # can be swaped to its position
                    ...
                    continue
                j = 1
                # print(j,cstack[j],allelems.count(cstack[j]),self._final_stack.count(cstack[j]))
                while j < len(cstack) and j <= reach and allelems.count(cstack[j]) == self._final_stack.count(cstack[j]):
                    j += 1
                if j <= reach and j < len(cstack): # can be swaped with garbage 
                    popcodes += ['SWAP' + str(j)]
                    ...
                    if len(cstack)+lpos <= reach and self._final_stack[lpos] in cstack[:lpos] :
                ...
                if j <= reach and j < len(cstack): # can be swaped with garbage 
                    ...
                    if len(cstack)+lpos >= reach:
                        ...
                        while  i < len(cstack) and i <= reach and (len(self._final_stack)-len(cstack)+i in solved or cstack[0] == cstack[i]):
                            i += 1
                        if i <= reach and i < len(cstack):
                            popcodes += ['SWAP' + str(i)]
                            popcodeids += ['SWAP' + str(i)]
                            # print(reg,cstack,self._final_stack)
                            cstack = [cstack[i]] + cstack[1:i] + [cstack[0]] + cstack[i + 1:]
...
def greedy_from_json(json_data: Dict[str, Any], verb=True, garbage=False, push_dup=2, reachable = 16) -> Tuple[
    ...
    try:
        ...
        if extend_tgt:
            ...
            # print("before pre:",encoding_ext._needed_in_stack_map,encoding_ext._initial_stack,encoding_ext._final_stack)
            ...
            # print("after pre:",instr_ext,encoding_ext._needed_in_stack_map,initial_ext,opcodeids_ini_ext,solved_ext)
