...
f = sys.argv[1]#"num_own_gas.txt"

ff = open(f, "r")
lines = ff.readlines()

origin = list(filter(lambda x: x.find("ORIGIN OWN COSTAG")!= -1, lines))
origin_number = list(map(lambda x: int(x.split(":")[-1].strip()), origin))

f_names = list(map(lambda x: x.split(":")[0].rstrip(" ORIGIN OWN COSTAG"),origin))

total_origin = sum(origin_number)


opt = list(filter(lambda x: x.find("OPT OWN COSTAG")!= -1, lines))
...
worse_file = open("worse_own_gas_contracts.txt", "w")
worse_file.write(s)
worse_file.close()

print()
print(" ===== OWN GAS STATISTICS =====")
print()
...
print("TOTAL")
print("TOTAL OWN GAS ORIGINAL: "+str(total_origin))
print("TOTAL OWN GAS OPT: "+str(total_opt))
print("%: "+str((total_opt/(total_origin*1.0))*100.0))
print()
print("MEJORAMOS")
print("TOTAL OWN GAS ORIGINAL: "+str(total_mejor))
print("TOTAL OWN GAS OPT: "+str(optimizado_mejor))
