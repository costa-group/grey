...
def update_dicts(final_dict, curr_dict):
    for k in curr_dict:
        final_dict[k] = final_dict.get(k,0)+curr_dict[k]
...
def execute_function(origin_file, log_opt_file):
    ...
    for c in evm_opt:
        ...
        update_dicts(num_ins_opt,opt[5])
        ...
        for cc in contracts:
            ...
            if c.strip() in json:
                ...
                update_dicts(num_ins_src, origin_ins[5])
    return (total_terminal, total_pops, all_pops_opt, total_sol_terminal, total_sol_pops, all_pops_sol, total_ins_terminal_opt, total_ins_terminal_sol, total_blocks_solc, total_blocks_opt, num_ins_opt, num_ins_src)
