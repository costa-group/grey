...
def get_bytes(ins):
    if ins.get("name","").startswith("PUSH"):
        return 1+len(ins.get("value",""))
    else:
        return 1
    
def extract_code_and_data(json_file):
    try:
        ...
        for key, value in data_field.items():
            data_code = value.get(".code", [])
            print(f"Clave: {key}, Code:")
            
            gas_data = 0
            bytes_data = 0
            for instruction in data_code:
                #print(instruction)
                gas_data += get_ins_cost(instruction.get("name",""))
                bytes_data += get_bytes(instruction)

            dict_data[key] = (gas_data, bytes_data, len(data_code))

        dict_data["code"] = (gas_code, bytes_code, len_code) 
        return dict_data
